...
every container-template XML on the USB flash** from scratch, with no caching, so the
...
**Measured on a busy server (well over 100 containers), real `getAllInfo()`:**
...
the whole table each second, which is heavy on CPU and on the client GPU. The plugin lets
...
- **Config-driven and idempotent**. `apply.sh` reads the saved config and makes the
...
Remove it from the **Plugins** page. That restores stock `DockerClient.php` and
`docker_load`.
